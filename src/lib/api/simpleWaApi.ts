...
// Ahora todo pasa por el orquestador /wa/*
...
// Helper para abrir SSE de eventos unificados (/wa/events/stream)
export function openWaEventsStream(
  onEvent: (ev: { type: string; data: any }) => void
) {
  const token = getAccessToken();
  if (!token) throw new Error("No auth token");
  const url = new URL("/wa/events/stream", window.location.origin);
